...
public class ParkYeriSaatleriAdapter extends RecyclerView.Adapter<ParkYeriSaatleriAdapter.SlotViewHolder> {
    ...
    public void onBindViewHolder(@NonNull SlotViewHolder slotViewHolder, int i) {
        ...
        slotViewHolder.mPlus.setOnClickListener(v -> {
            ...
            if (spotsAvailable > a) {
                ...
                GooglePinParkYeriKiralama.noOfSlotsToBeBooked = "" + (Integer.parseInt(slot.get("booked").toString()) + Integer.parseInt(slotViewHolder.mNoOfSlots.getText().toString()));
            }
        });
    ...
    void updateData(SlotViewHolder slotViewHolder, int i) {
        ...
        GooglePinParkYeriKiralama.AmountToPay.setText("" + amount + " /-");
        GooglePinParkYeriKiralama.AmountToPayLayout.setVisibility(View.VISIBLE);

        String slotNo = "slot"+ (i+1);

        ParkingBooking parkingBooking = GooglePinParkYeriKiralama.parkingBooking;
